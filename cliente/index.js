...
app.get('/data/:zoom', function (req, res, next) {
  ...
  var bbox = req.query.bbox.split(','),
      ...
      query = solr.createQuery().q('*.*').rows(0).rangeFilter(range);

  // Check how many features Solr would return directly
  solr.search(query, function (err, result) {
      //if (result != null )
      {
    console.log(query.toString()+'<- ' + bbox + ': Features:' + result.response.numFound);
    if (result.response.numFound < 3000) {
      ...
      solr.search(query, function (err, result) {
        // Convert to GeoJSON FeatureCollection
          //console.log(result);
          //console.log('Second Console Starts here' + result.response.docs);
