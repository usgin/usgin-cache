(function() {
  ...
  var ngdsCache = this.cacheDemo.ngdsCache = function (options) {
    ...
    var map = this.cacheDemo.map = L.map('map').setView([40.346, ], 5);
  /*
  ...
  ngdsCache({
    ...
    onEachFeature: function (f, layer) {
      ...
      //_.each(_.keys(f.properties), function (propName) {
        _.every(_.keys(f.properties), function (propName,i) {
        html += '<tr><th align="left">' + propName + '</th><td>' + f.properties[propName] + '</td></tr>';
        return i != 6;
      });
      html += '</table>';
